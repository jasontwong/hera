  ...
  $(function() {
    var spinnerOptions = { color: '#aaaaaa', width: 2, radius: 5 },
      ...
      size = parseInt(window
          .getComputedStyle(statusContainer.parentNode)
      ...
      showUsers = function (data) {
        ...
        /* Set up charts. */
        var surveysChart = dc.barChart("#users-surveys-chart"),
          ...
          /* Set up dimensions. */
          surveys = users.dimension(function(d) { return d.surveys; }),
          surveysCount = surveys
            .group()
            .reduceCount(),
          maxSurveys = surveysCount.top(1)[0].value,
          rewards = users.dimension(function(d) { return d.rewards; }),
          rewardsCount = rewards
            .group()
            .reduceCount(),
          maxRewards = rewardsCount.top(1)[0].value,
          stores = users.dimension(function(d) { return d.stores; }),
          storesCount = stores
            .group()
            .reduceCount(),
        ...
        surveysChart.width(480)
          ...
          .y(d3.scale.pow().exponent(0.3).domain([0, maxSurveys]))
          .yAxis().ticks(6);
        ...
        rewardsChart.width(480)
          ...
          .gap(1)
          .x(d3.scale.linear().domain([0, 50]))
          .y(d3.scale.pow().exponent(0.3).domain([0, maxRewards]))
          .yAxis().ticks(6);
        ...
        storesChart.width(480)
          ...
          .gap(1)
          .x(d3.scale.linear().domain([0, 50]))
          .y(d3.scale.pow().exponent(0.3).domain([0, maxVisits]))
          .yAxis().ticks(6);
